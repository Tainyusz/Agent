...
/** 语音输入供应商 */
export type VoiceDictationProvider = 'custom' | 'doubao'
...
/** 上传音频转写参数 */
export interface VoiceDictationTranscribeAudioInput {
  sessionId: string
  data: ArrayBuffer
  mimeType?: string
  filename?: string
...
/** URL 转写参数 */
export interface VoiceDictationTranscribeUrlInput {
  sessionId?: string
  url: string
}

/** 转写分段 */
export interface VoiceDictationSegment {
  start: number
  end: number
  text: string
  speaker?: number
...
/** 转写结果 */
export interface VoiceDictationTranscribeResult {
  text: string
  duration?: number
  segments?: VoiceDictationSegment[]
}
...
export const VOICE_DICTATION_IPC_CHANNELS = {
  ...
  /** 上传音频并转写 */
  TRANSCRIBE_AUDIO: 'voice-dictation:transcribe-audio',
  /** 传入 URL 并转写 */
  TRANSCRIBE_URL: 'voice-dictation:transcribe-url',
  /** 输出最终文本 */
