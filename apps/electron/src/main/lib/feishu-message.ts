...
export function buildAgentReplyCard(result: FormattedAgentResult, subtitle?: string): Record<string, unknown> {
  ...
  return {
    ...
    header: {
      title: { tag: 'plain_text', content: '无限轻松助手' },
...
export function buildNotificationCard(
  ...
  return {
    config: { wide_screen_mode: true },
    header: {
      title: { tag: 'plain_text', content: '无限轻松任务完成' },
      template: 'green',
    },
    elements: [
      {
      ...
      ...(toolLine ? [
        ...
        {
          tag: 'note',
          elements: [{ tag: 'plain_text', content: `${toolLine} | 在无限轻松中查看完整回复` }],
        },
      ] : []),
    ],
  }
...
export function buildErrorCard(errorMessage: string): Record<string, unknown> {
  return {
    config: { wide_screen_mode: true },
    header: {
      title: { tag: 'plain_text', content: '无限轻松错误' },
      template: 'red',
    },
    elements: [
...
export function buildHelpCard(): Record<string, unknown> {
  return {
    config: { wide_screen_mode: true },
    header: {
      title: { tag: 'plain_text', content: '无限轻松 Bot 命令' },
      template: 'blue',
    },
    elements: [
      {
