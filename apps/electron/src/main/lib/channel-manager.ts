...
function normalizeLegacyProvider(channel: Channel): Channel {
  if (channel.provider !== 'anthropic') return channel

  const baseUrl = channel.baseUrl.toLowerCase()
  const modelText = channel.models.map((model) => `${model.id} ${model.name}`).join(' ').toLowerCase()

  let provider: ProviderType | null = null
  if (baseUrl.includes('api.deepseek.com') || modelText.includes('deepseek')) {
    provider = 'deepseek'
  } else if (baseUrl.includes('api.kimi.com/coding')) {
    provider = 'kimi-coding'
  } else if (baseUrl.includes('api.moonshot.cn') || modelText.includes('kimi')) {
    provider = 'kimi-api'
  }
...
function normalizeLegacyProviders(config: ChannelsConfig): boolean {
  let changed = false
  config.channels = config.channels.map((channel) => {
    const normalized = normalizeLegacyProvider(channel)
    if (normalized.provider !== channel.provider) {
      changed = true
    }
    return normalized
  })
...
export function listChannels(): Channel[] {
  ...
  const normalized = normalizeLegacyProviders(config)
...
export async function testChannel(channelId: string): Promise<ChannelTestResult> {
  ...
  try {
    switch (channel.provider) {
      ...
      default:
        return { success: false, message: `不支持的供应商: ${channel.provider}。请检查模型供应商配置。` }
    }
  } catch (error) {
