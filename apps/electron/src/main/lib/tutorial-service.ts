...
export function createWelcomeConversation(): ConversationMeta | null {
  ...
  try {
    // 1. 创建对话
    const meta = createConversation('了解无限轻松')

    // 2. 保存教程文件为附件
    const attachmentId = randomUUID()
    const attachmentFilename = '无限轻松使用教程.md'
    ...
    const userMessage: ChatMessage = {
      id: randomUUID(),
      role: 'user',
      content: '请帮我了解无限轻松的功能和使用方式，我附上了完整的使用教程作为参考。',
      createdAt: now,
    ...
    const assistantMessage: ChatMessage = {
      id: randomUUID(),
      role: 'assistant',
      content: `你好，欢迎使用无限轻松！我已经阅读了完整的使用教程。你可以向我提问来快速了解无限轻松，比如：
