...
import type { AgentSendInput, AgentMessage, AgentGenerateTitleInput, AgentProviderAdapter, TypedError, RetryAttempt, SDKMessage, SDKAssistantMessage, AgentStreamPayload, RewindSessionResult, SdkBeta, ProviderType, Channel } from '@proma/shared'
import {
  SAFE_TOOLS,
...
/** 标题生成 Prompt */
const TITLE_PROMPT = '根据下面的 Agent 对话内容，生成一个简短、具体的会话标题（10字以内）。只输出标题，不要有任何解释、标点符号或引号。\n\n对话内容：'

/** 短消息阈值：低于此长度直接使用原文作为标题 */
const SHORT_TITLE_MESSAGE_THRESHOLD = 12
...
interface TitleTarget {
  ...
  apiKey: string
  modelId: string
  source: 'deepseek' | 'current'
}

function isDeepSeekChannel(channel: Channel): boolean {
  return channel.provider === 'deepseek' ||
    channel.baseUrl.includes('api.deepseek.com') ||
    channel.models.some((model) => model.id.toLowerCase().includes('deepseek'))
}

function pickDeepSeekTitleModel(channel: Channel): string {
  const enabledModels = channel.models.filter((model) => model.enabled)
  return enabledModels.find((model) => model.id === 'deepseek-v4-flash')?.id
    ?? enabledModels.find((model) => model.id.toLowerCase().includes('flash'))?.id
    ?? enabledModels.find((model) => model.id.toLowerCase().includes('deepseek-v4'))?.id
    ?? enabledModels[0]?.id
    ?? 'deepseek-v4-flash'
}

function pickChannelModel(channel: Channel, requestedModelId: string): string {
...
function decryptTitleApiKey(channelId: string): string | null {
  try {
    const apiKey = decryptApiKey(channelId).trim()
    return apiKey || null
  } catch {
    return null
  }
...
function selectTitleTarget(
  channels: Channel[],
  currentChannelId: string,
  currentModelId: string,
): TitleTarget | null {
  const deepSeekCandidates = channels
    .filter(isDeepSeekChannel)
    .sort((a, b) => Number(b.enabled) - Number(a.enabled))

  for (const channel of deepSeekCandidates) {
    const apiKey = decryptTitleApiKey(channel.id)
    if (!apiKey) continue
    return {
      channel,
      apiKey,
      modelId: pickDeepSeekTitleModel(channel),
      source: 'deepseek',
    }
  }
  ...
  const apiKey = decryptTitleApiKey(currentChannel.id)
  if (!apiKey) return null

  return {
    channel: currentChannel,
    apiKey,
    modelId: pickChannelModel(currentChannel, currentModelId),
    source: 'current',
  }
}

function cleanGeneratedTitle(title: string): string | null {
  const cleaned = title
    .trim()
    .replace(/^["'""''「《]+|["'""''」》]+$/g, '')
    .replace(/[。！？!?,，、；;：:]+$/g, '')
...
function fallbackTitleFromMessage(userMessage: string): string | null {
  const normalized = userMessage
    .replace(/```[\s\S]*?```/g, ' ')
    .replace(/<[^>]+>/g, ' ')
    .replace(/\s+/g, ' ')
    .trim()
  return cleanGeneratedTitle(normalized)
}
...
export class AgentOrchestrator {
  ...
  async generateTitle(input: AgentGenerateTitleInput): Promise<string | null> {
    ...
    const fallbackTitle = fallbackTitleFromMessage(userMessage)
    if (userMessage.trim().length <= SHORT_TITLE_MESSAGE_THRESHOLD) {
      console.log('[Agent 标题生成] 消息较短，直接使用原文标题:', fallbackTitle)
      return fallbackTitle
    }

    try {
      const channels = listChannels()
      const target = selectTitleTarget(channels, channelId, modelId)
      if (!target) {
        console.warn('[Agent 标题生成] 没有可用的标题生成渠道，使用兜底标题:', fallbackTitle)
        return fallbackTitle
      }

      console.log('[Agent 标题生成] 使用标题渠道:', {
      ...
      const providerAdapter = getAdapter(target.channel.provider)
      const request = providerAdapter.buildTitleRequest({
        baseUrl: target.channel.baseUrl,
      ...
      const result = cleanGeneratedTitle(title) ?? fallbackTitle

      console.log(`[Agent 标题生成] 生成标题成功: "${result}"`)
      return result
    } catch (error) {
      console.warn('[Agent 标题生成] 生成失败，使用兜底标题:', error)
      return fallbackTitle
    }
  }
  ...
  async sendMessage(input: AgentSendInput, callbacks: SessionCallbacks): Promise<void> {
    ...
    const ensureTitleGenerated = async (): Promise<void> => {
      if (titleGenerationStarted) return
      titleGenerationStarted = true
      await this.autoGenerateTitle(sessionId, userMessage, channelId, resolvedModel, callbacks)
    }
    ...
    try {
      ...
      await ensureTitleGenerated().catch((err) => {
        console.warn('[Agent 编排] finally 兜底标题生成失败:', err)
      })
      ...
      // AskUserQuestion 的生命周期由用户交互决定；只在删除会话时清理。
