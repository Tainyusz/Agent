...
export async function pasteTextAtCurrentCursor(text: string): Promise<TextInsertionResult> {
  ...
  try {
    ...
    return { success: true, mode: 'cursor', message: '已写入当前光标位置' }
  } catch (error) {
...
function captureClipboardBuffers(): ClipboardBufferSnapshot[] {
  ...
  for (const format of clipboard.availableFormats()) {
    try {
      ...
      if (buffer.byteLength > 0) snapshots.push({ format, buffer })
    } catch {
      // 系统私有剪贴板格式可能无法读取，跳过即可。
    }
  }
  return snapshots
}
...
async function triggerMacPaste(): Promise<void> {
  if (!systemPreferences.isTrustedAccessibilityClient(true)) {
    throw new Error('需要在 macOS 系统设置中允许无限轻松使用辅助功能')
  }
...
async function triggerWindowsPaste(): Promise<void> {
  await execFileAsync(
    'powershell.exe',
    ['-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'Bypass', '-Command', WINDOWS_SEND_INPUT_SCRIPT],
...
public static class WuxianQingsongKeyboardInput
{
...
[WuxianQingsongKeyboardInput]::Paste()
`
