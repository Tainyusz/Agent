...
export function registerIpcHandlers(): void {
  ...
  // 上传当前对话并生成分享链接
  ipcMain.handle(
    CHAT_IPC_CHANNELS.SHARE_CONVERSATION,
    async (_, conversationId: string): Promise<ChatShareResult> => {
      return shareConversation(conversationId)
    }
  )
  ...
  // 上传当前 Agent 会话并生成分享链接
  ipcMain.handle(
    AGENT_IPC_CHANNELS.SHARE_SESSION,
    async (_, sessionId: string): Promise<ChatShareResult> => {
      return shareAgentSession(sessionId)
    }
  )
  ...
  ipcMain.handle(
    ...
    async (_, updates: VoiceDictationSettingsUpdate): Promise<VoiceDictationSettings> => {
      ...
      const { createVoiceDictationWindow, destroyVoiceDictationWindow } = await import('./lib/voice-dictation-window')
      const { reregisterAllGlobalShortcuts } = await import('./lib/global-shortcut-service')
      const saved = updateVoiceDictationSettings(updates)
      if (saved.enabled) {
  ...
  ipcMain.handle(
    ...
    async (): Promise<VoiceDictationTestResult> => {
      const { testCustomTranscribeConnection } = await import('./lib/custom-transcribe-service')
      return testCustomTranscribeConnection()
    }
  )
  ...
  ipcMain.handle(
    ...
    async (_event, _input: VoiceDictationStartInput): Promise<void> => {
      // 自定义接口是录完后上传文件识别，不需要提前建立 ASR 连接。
    }
  )
  ...
  ipcMain.handle(
    ...
    async (): Promise<void> => {
      // 兼容流式 UI API，当前自定义接口不使用分片上传。
    }
  )

  ipcMain.handle(
    VOICE_DICTATION_IPC_CHANNELS.STOP,
    async (_event, _input: VoiceDictationStopInput): Promise<void> => {
      // 录音停止由渲染进程完成。
    }
  )
  ...
  ipcMain.handle(
    VOICE_DICTATION_IPC_CHANNELS.TRANSCRIBE_AUDIO,
    async (_, input: VoiceDictationTranscribeAudioInput): Promise<VoiceDictationTranscribeResult> => {
      const { transcribeVoiceAudio } = await import('./lib/custom-transcribe-service')
      return transcribeVoiceAudio(input)
    }
  )

  ipcMain.handle(
    VOICE_DICTATION_IPC_CHANNELS.TRANSCRIBE_URL,
    async (_, input: VoiceDictationTranscribeUrlInput): Promise<VoiceDictationTranscribeResult> => {
      const { transcribeVoiceUrl } = await import('./lib/custom-transcribe-service')
      return transcribeVoiceUrl(input)
    }
  )
  ...
  ipcMain.handle('migration:openFileDialog', async () => {
    ...
    const result = await dialog.showOpenDialog({
      title: '选择迁移文件',
      filters: [
        { name: '无限轻松迁移文件', extensions: ['proma-backup', 'proma-share'] },
        { name: '所有文件', extensions: ['*'] },
      ],
      properties: ['openFile'],
    })
  ...
  ipcMain.handle('migration:saveFileDialog', async (_, mode: string) => {
    ...
    const result = await dialog.showSaveDialog({
      ...
      filters: [
        { name: mode === 'personal' ? '无限轻松个人备份' : '无限轻松分享包', extensions: [ext] },
      ],
    })
