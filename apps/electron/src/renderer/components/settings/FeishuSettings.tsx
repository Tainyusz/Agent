...
function FeishuCliSection(): React.ReactElement {
  ...
  return (
    <SettingsSection
      title="配置飞书 CLI"
      description="飞书官方开源的命令行工具，配置后无限轻松将可以直接读消息、查日历、写文档、建多维表格、发邮件，把任务真正落到飞书里完成。"
    >
      <SettingsCard divided={false}>
        <div className="px-4 py-4 space-y-2 text-sm text-muted-foreground">
          <p className="text-xs">复制配置提示词，并前往飞书Bot日常绑定的<strong>工作区</strong>，创建新的无限轻松对话并发送即可让无限轻松协助完成配置。</p>
          <button
            type="button"
            className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors cursor-pointer"
            onClick={() => setExpanded(!expanded)}
...
function FeishuBindingsTab(): React.ReactElement {
  ...
  return (
    <div className="space-y-8">
      <SettingsSection
        title="绑定管理"
        description="查看和管理飞书聊天与无限轻松工作区/会话的绑定关系"
        action={
          <Button
            size="sm"
            variant="outline"
