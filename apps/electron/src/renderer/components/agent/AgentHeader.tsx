...
import { Pencil, Check, X, PanelRight, Share2, Loader2 } from 'lucide-react'
...
export function AgentHeader({ sessionId }: AgentHeaderProps): React.ReactElement | null {
  ...
  /** 上传当前 Agent 会话并复制分享链接 */
  const handleShare = async (): Promise<void> => {
    if (sharing) return
    setSharing(true)
    try {
      const result = await window.electronAPI.shareAgentSession(session.id)
      await navigator.clipboard.writeText(result.url)
      toast.success('分享链接已复制', {
        description: result.url,
      })
    } catch (error) {
      console.error('[AgentHeader] 分享 Agent 会话失败:', error)
        ...
        <>
          ...
          <Tooltip>
            ...
            <TooltipContent side="bottom">
              <p>{sharing ? '正在生成分享链接' : '分享 Agent 会话'}</p>
            </TooltipContent>
          </Tooltip>
