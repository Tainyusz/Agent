...
import { Pencil, Check, X, Pin, Columns2, Share2, Loader2 } from 'lucide-react'
...
export function ChatHeader({ conversation }: ChatHeaderProps): React.ReactElement | null {
  ...
  const handleShare = async (): Promise<void> => {
    ...
    setSharing(true)
    try {
      const result = await window.electronAPI.shareConversation(conversation.id)
      await navigator.clipboard.writeText(result.url)
      toast.success('分享链接已复制', {
        description: result.url,
      })
    } catch (error) {
      console.error('[ChatHeader] 分享对话失败:', error)
    ...
    <div className="relative z-[51] flex items-center gap-2 px-4 h-[48px] titlebar-drag-region">
      ...
      <div className="flex items-center gap-1 titlebar-no-drag ml-auto">
        ...
        <Tooltip>
          ...
          <TooltipContent side="bottom"><p>{sharing ? '正在生成分享链接' : '分享对话'}</p></TooltipContent>
        </Tooltip>
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
