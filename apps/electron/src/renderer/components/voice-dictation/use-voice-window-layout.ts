...
export interface VoiceWindowLayoutResult {
...
export function useVoiceWindowLayout(input: VoiceWindowLayoutInput): VoiceWindowLayoutResult {
  ...
  const scrollTranscriptToBottom = React.useCallback(() => {
    ...
    transcriptBox.scrollTop = Math.max(0, transcriptBox.scrollHeight - transcriptBox.clientHeight)
  }, [])
  ...
  const resizeVoiceWindow = React.useCallback(() => {
    ...
    const maxWindowHeight = Math.max(220, Math.min(MAX_WINDOW_HEIGHT, window.screen.availHeight - 24))
