...
/**
 * 根据 Base URL 和供应商类型获取渠道 Logo
 *
 * 明确选择了内置供应商时，优先显示供应商图标，避免兼容代理地址误命中 URL 图标规则。
 * 自定义渠道按 URL 域名匹配实际供应商，未匹配到则返回默认图标。
 *
 * @param baseUrl 渠道的 Base URL
 * @param provider 渠道的供应商类型
 */
export function getChannelLogo(baseUrl: string, provider?: ProviderType): string {
